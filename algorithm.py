...
class EigenGameEstimator(BaseEstimator):
    ...
    #functions fo mu-EigenGame

    def _estimate_next_component_mu_EigenGame(self, X, k):
        n = X.shape[1]
        v = np.random.random((n,))
        v /= np.linalg.norm(v)
        self.V[:,k] = v
        ...
        for t in range(self.n_iter):
            du = self._grad_mu_EigenGame(self.V[:,:k], v, X)
            du = du - np.dot(du, v)*v
            v_new = v + 1/(t+1)*du
            v_new /= np.linalg.norm(v_new)
            self.V[:,k] = v_new
    ...
    def _grad_mu_EigenGame(self, V, v, X):
        M = X.T@X
        if V is None:
            return 2*M.dot(v)
        else:
            revards, penalties = 0, 0
            columns = V.shape[1]
            for j in range(columns):
                revards += 1/X.shape[0]*M.dot(v)
                penalties += 1/X.shape[0]*np.dot(v, np.dot(M,V[:,j]))*V[:,j]
            du = revards - penalties
            return du

    def _estimate_mu_EigenGame(self, X, V_true=None):
        # Handle number of components to obtain
        if self.n_components is None:
            n_components = X.shape[1]
        else:
        ...
        for k in range(n_components):
            self._estimate_next_component_mu_EigenGame(X, k)
